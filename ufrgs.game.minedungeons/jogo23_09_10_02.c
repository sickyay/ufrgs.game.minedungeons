...
#define MAX_ARCHIVE_NAME 30 //maximo de caracteres no nome do arcuivo
...
bool draw_menu (int selected_option, Texture2D texture[], Font font)
{
    ...
    int pixels_offset[2][6] = {{85,106,80,80,90,80},{85,130,130,130,130,130}};
    ...
    if(selected_option>=0&&selected_option<=5){
        for(i = 0;i<6;i++){
            ...
            DrawTextEx(font,options_text[i], (Vector2){ (MAP_PIXEL_WIDTH/2) - (MENU_BUTTON_PIXEL_WIDTH/2)+pixels_offset[0][i], (button_height)*i+MENU_PIXEL_BORDER+button_height/4}, button_height/2,0, WHITE);
        }
    }else{
        for(i = 0;i<6;i++){
            if(i!=0){
            ...
            DrawTextEx(font,saves_text[i], (Vector2){ (MAP_PIXEL_WIDTH/2) - (MENU_BUTTON_PIXEL_WIDTH/2)+pixels_offset[1][i], (button_height)*i+MENU_PIXEL_BORDER+button_height/4}, button_height/2,0, WHITE);
        }
    }
    return true;
}
...
bool draw_hud (struct STRUCT_DATA *data, Texture2D texture[], Font font)
{
    ...
    for(i=1; i<=(PLAYER_HP/2); i++)
    {
        ...
        {
            DrawTexture(texture[17],10+((i-1)*(60+5)),MAP_PIXEL_HEIGHT+10,WHITE);//com vida dourada
        }
        else
        {
            if(i*2<=data->player.health_points)
            {
                DrawTexture(texture[16],10+((i-1)*(60+5)),MAP_PIXEL_HEIGHT+10,WHITE);//com vida
            }
            else if((i*2)-1==data->player.health_points&&data->player.health_points%2!=0)
            {
                DrawTexture(texture[22],10+((i-1)*(60+5)),MAP_PIXEL_HEIGHT+10,WHITE);//com meia vida
            }
            else
            {
    ...
    DrawTexture(texture[23],400,MAP_PIXEL_HEIGHT+10,WHITE);//sem vida
    DrawTextEx(font,TextFormat("%1i", data->bomb_inventory), (Vector2){450,MAP_PIXEL_HEIGHT+40},30,0,WHITE);//vida do player

    DrawTextEx(font,TextFormat("Level: %1i", data->level+1), (Vector2){530,MAP_PIXEL_HEIGHT+20},40,0,WHITE);//vida do player

    data->seconds = (int)data->elapsedTime;
    data->minutes = data->seconds / 60;
    data->seconds %= 60;

    DrawTextEx(font,TextFormat("%02i:%02i",data->minutes,data->seconds), (Vector2){900,MAP_PIXEL_HEIGHT+20},40,0,WHITE);//vida do player
...
bool save_data(struct STRUCT_DATA data,int selected_save)
{
    ...
    int flag;
    FILE *arc;
    char archive_name[MAX_ARCHIVE_NAME] = "resources/saves/save0.bin";
    archive_name[20] = selected_save + '0';
...
bool load_data(struct STRUCT_DATA *data,int selected_save)
{
    ...
    data->startTime = difftime(data->currentTime,data->elapsedTime);
...
bool place_bomb(struct STRUCT_DATA *data, struct STRUCT_STATS *placer,int initial_x,int initial_y, Sound sounds[])
{
...
{
    archive_name[20] = (level%10) + '0';
    archive_name[19] = ((level%100)/10) + '0';
    archive_name[18] = (level/100) + '0';
}
...
int main(void)
{
    ...
    char map_file_name[MAX_ARCHIVE_NAME] = "resources/maps/map000.txt";//nome da versao do mapa que estamos jogando
    bool menu_open = true;
    time_t finishTime = 0;

    Texture2D textures[25]; //Carrega todas as texturas do jogo e armazena em um vetor
    ...
    textures[22] = LoadTexture("resources/textures/halfhearthhud.png");
    textures[23] = LoadTexture("resources/textures/tnthud.png");

    Sound sounds[18]; //Carrega todos os sons do jogo e armazena em um vetor
    ...
    sounds[17] = LoadSound("resources/sounds/toplacetnt.wav");

    Font font = LoadFont("resources/font/font.ttf"); ////Carrega a fonte do jogo
    ...
    while (!WindowShouldClose()){//Roda enquanto não for fechada a tela
        ...
        time(&data.currentTime); // Obtém o tempo atual
        data.elapsedTime = difftime(data.currentTime, data.startTime);
        ...
        if(!IsSoundPlaying(sounds[15]))
            PlaySound(sounds[15]);
                    ...
                    if(!IsSoundPlaying(sounds[0]) && !IsSoundPlaying(sounds[1]) && !IsSoundPlaying(sounds[2])){

            switch(rand()%1001){
                case 300:  PlaySound(sounds[0]);
                    break;
                case 500:  PlaySound(sounds[1]);
                    break;
                case 800:  PlaySound(sounds[2]);
                    break;
            }
        }
        ...
        if (!menu_open) //Roda enquanto o menu não for aberto
        {
            ...
            if(data.player.head_direction_x!=0||data.player.head_direction_y!=0)//se alguma das direções não for nula
            {
                if(data.intends_to_put_bomb==true&&data.bomb_inventory>0){
                    if(place_bomb(&data,&data.player, data.player.head_direction_x, data.player.head_direction_y, sounds)) data.intends_to_put_bomb = false;
                }else{
        ...
        if(menu_open){
            ...
            if(data.level==10){
                if(finishTime==0){
                    finishTime = data.elapsedTime;
                }
                DrawRectangle(0,0,MAP_PIXEL_WIDTH,MAP_PIXEL_HEIGHT,(Color){0,0,0,190});//limpa a tela e define cor de fundo
                data.seconds = (int)finishTime;
                data.minutes = data.seconds / 60;
                data.seconds %= 60;

                DrawTextEx(font,"You Survided! it took you:", (Vector2){(MAP_PIXEL_WIDTH/2)-290, (MAP_PIXEL_HEIGHT/2)-80}, 50,0, WHITE);
                DrawTextEx(font,TextFormat("%02i:%02i",data.minutes,data.seconds), (Vector2){(MAP_PIXEL_WIDTH/2)-90, (MAP_PIXEL_HEIGHT/2)},60,0,WHITE);//vida do player
                DrawTextEx(font,"Press M to open Menu", (Vector2){(MAP_PIXEL_WIDTH/2)-260, (MAP_PIXEL_HEIGHT/2)+80}, 50,0, WHITE);
            }
        }
