/*
terminar scan_map
aprender a usar time.h para realizar os cooldowns direito sem gambiarra
função pra resetar parametros da struct_status de tudo
função para trap dar dano
...
#define PLAYER_SHOOT_COOLDOWN 10 //coldown do tiro do player
...
int draw_map (char map_matrix[MAP_LINES][MAP_COLUMNS+1],struct STRUCT_STATS object, struct STRUCT_STATS object1, struct STRUCT_STATS trap_vector[])//funcao que le o que acontece na matriz do mapa
{
...
int scan_map (char matrix[MAP_LINES][MAP_COLUMNS+1],struct STRUCT_STATS *player_variable, struct STRUCT_STATS enemy_vector[], struct STRUCT_STATS projectile_vector[], struct STRUCT_STATS wall_vector[], struct STRUCT_STATS trap_vector[])//funcao que le o que acontece na matriz do mapa
{
    ...
    for (int y = 0; y < MAP_LINES; y++)//colunas
    {
        for (int x = 0; x < MAP_COLUMNS; x++)//linhas
        {
            ...
            case '#'://parede
                ...
                wall_vector[wall_counter].health_points = 5;
                wall_counter++;
                break;
            case 'I'://inimigo
...
int main(void)
{
    ...
    struct STRUCT_STATS player = {0,0,0,0,0,0,0,PLAYER_STEP_COOLDOWN,0,PLAYER_SHOOT_COOLDOWN,0};
    struct STRUCT_STATS projectile[MAX_PROJECTILES_ON_SCREEN];
    ...
    scan_map(map,&player,enemy,projectile,wall,trap);
