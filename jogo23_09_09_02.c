...
bool draw_map (struct STRUCT_DATA *data, Texture2D texture[], Font font)//funcao que le o que acontece na matriz do mapa
{
    ...
    ClearBackground(PINK);//limpa a tela e define cor de fundo
...
bool draw_menu (int selected_option, Font font)
{
    ...
    if(selected_option>=0&&selected_option<=5){
        for(i = 0;i<6;i++){
            ...
            DrawTextEx(font,options_text[i], (Vector2){ (MENU_PIXEL_BORDER)+button_height/6, (button_height)*i+MENU_PIXEL_BORDER+button_height/4}, button_height/2,0, RED);
        }
    }else{
        for(i = 0;i<6;i++){
            ...
            DrawTextEx(font,saves_text[i], (Vector2){(MENU_PIXEL_BORDER)+button_height/6, (button_height)*i+MENU_PIXEL_BORDER+button_height/4}, button_height/2,0, RED);
        }
    }
    return true;
}
        ...
        if(data->stream[i].x>=0&&data->stream[i].x<MAP_COLUMNS&&data->stream[i].y>=0&&data->stream[i].y<MAP_LINES){
            if(data->stream[i].lifetime.timer_current - data->stream[i].lifetime.timer_start <= data->stream[i].lifetime.cooldown){
                ...
                switch(data->stream[i].type){
                    case 0:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y+(SIDE/2),effect_pos_x+(SIDE/2) ,effect_pos_y+SIDE , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        data->stream[i].type = data->stream[i].type + 1;
                        break;
                    case 1:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+SIDE , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        break;
                    case 2:
                        DrawLine(effect_pos_x, effect_pos_y+SIDE,effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        data->stream[i].type = data->stream[i].type + 1;
                        break;
                    case 3:
                        DrawLine(effect_pos_x+SIDE, effect_pos_y,effect_pos_x ,effect_pos_y+SIDE , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        break;
                    case 4:
                        DrawLine(effect_pos_x, effect_pos_y+(SIDE/2),effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        data->stream[i].type = data->stream[i].type + 1;
                        break;
                    case 5:
                        DrawLine(effect_pos_x, effect_pos_y+(SIDE/2),effect_pos_x+SIDE ,effect_pos_y+(SIDE/2) , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        break;
                    case 6:
                        DrawLine(effect_pos_x, effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        data->stream[i].type = data->stream[i].type + 1;
                        break;
                    case 7:
                        DrawLine(effect_pos_x, effect_pos_y,effect_pos_x+SIDE ,effect_pos_y +SIDE, (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        break;
                    case 8:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        data->stream[i].type = data->stream[i].type + 1;
                        break;
                    case 9:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+SIDE , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        break;
                    case 10:
                        DrawLine(effect_pos_x+SIDE, effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        data->stream[i].type = data->stream[i].type + 1;
                        break;
                    case 11:
                        DrawLine(effect_pos_x+SIDE, effect_pos_y,effect_pos_x ,effect_pos_y+SIDE , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        break;
                    case 12:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y+(SIDE/2),effect_pos_x +SIDE,effect_pos_y+(SIDE/2) , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        data->stream[i].type = data->stream[i].type + 1;
                        break;
                    case 13:
                        DrawLine(effect_pos_x, effect_pos_y+(SIDE/2),effect_pos_x+SIDE ,effect_pos_y+(SIDE/2) , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        break;
                    case 14:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y+(SIDE/2),effect_pos_x+SIDE ,effect_pos_y+SIDE , (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
                        data->stream[i].type = data->stream[i].type + 1;
                        break;
                    case 15:
                        DrawLine(effect_pos_x, effect_pos_y,effect_pos_x+SIDE ,effect_pos_y +SIDE, (Color){255, 140, 0, 255-(((data->stream[i].lifetime.timer_current-data->stream[i].lifetime.timer_start)*255)/data->stream[i].lifetime.cooldown)});
...
int main(void)
{
    ...
    Sound background = LoadSound("resources/sounds/background.wav");
    Sound sound1 = LoadSound("resources/sounds/sound1.wav");
    Sound sound2 = LoadSound("resources/sounds/sound2.wav");
    Sound sound3 = LoadSound("resources/sounds/sound3.wav");
            ...
            for(i=0; i<MAX_ENEMIES1; i++)//percorre o vetor de inimigos
            {
                ...
                if (data.enemy1[i].step.timer_current - data.enemy1[i].step.timer_start >= data.enemy1[i].step.cooldown)//se o cooldown for zero, pega a intenção de movimento
                {
                    ...
                    else if((data.enemy1[i].x>=0 && data.enemy1[i].x<MAP_COLUMNS && data.enemy1[i].y>=0 && data.enemy1[i].y<MAP_LINES))
                    {
                        ...
                        {
                            damage_position(&data,data.enemy1[i].x+data.enemy1[i].dx,data.enemy1[i].y+data.enemy1[i].dy,1);
                            data.enemy1[i].action.timer_start = clock();
                        }
            ...
            for(i=0; i<MAX_ENEMIES2; i++)//percorre o vetor de inimigos
            {
                ...
                if (data.enemy2[i].step.timer_current - data.enemy2[i].step.timer_start >= data.enemy2[i].step.cooldown)//se o cooldown for zero, pega a intenção de movimento
                {
                    ...
                    else if((data.enemy2[i].x>=0 && data.enemy2[i].x<MAP_COLUMNS && data.enemy2[i].y>=0 && data.enemy2[i].y<MAP_LINES))
                    {
                        ...
                        {
                            damage_position(&data,data.enemy2[i].x+data.enemy2[i].dx,data.enemy2[i].y+data.enemy2[i].dy,1);
                ...
                if(test_if_dead(&data,&data.enemy2[i])){
                    explode(&data,past_x,past_y,CREEPER_RADIUS,180,CREEPER_DAMAGE);
                }
            }
