...
#define ENEMY_STEP_COOLDOWN 10 //coldown do movimento do player
...
int draw_map (char matrix[MAP_LINES][MAP_COLUMNS+1],struct STRUCT_STATS object)//funcao que le o que acontece na matriz do mapa
{
    ...
    for (int y = 0; y < MAP_LINES; y++)//colunas
    {
        for (int x = 0; x < MAP_COLUMNS; x++)//linhas
        {
            ...
            case 'X'://fogueira
                ...
                break;
            case 'o'://tiro
                DrawRectangle((x*SIDE)+(SIDE/4),(y*SIDE)+(SIDE/4), SIDE/2, SIDE/2, BLUE);
                break;
            }
        }
    }
...
bool test_if_dead(struct STRUCT_STATS *object, char map_matrix[MAP_LINES][MAP_COLUMNS+1])
{
    bool flag = 0;
...
void damage_position(int x_position, int y_position, int damage, struct STRUCT_STATS *object1, struct STRUCT_STATS object2[], struct STRUCT_STATS object3[])//funcao que movimenta os objetos na matriz
{
    int i;

    if(object1->x == x_position && object1->y == y_position && object1->health_points>=1)//se a posição onde o projétil queria se mover é a mesma do inimigo
    {
        object1->health_points = object1->health_points - damage;//decrementa da vida do inimigo
    }

    for(i=0; i<MAX_ENEMIES; i++)//percorre o vetor de inimigos
    {
        if(object2[i].x == x_position && object2[i].y == y_position && object2[i].health_points>=1)//se a posição onde o projétil queria se mover é a mesma do inimigo
        {
            object2[i].health_points = object2[i].health_points - damage;//decrementa da vida do inimigo
        }
    }
    for(i=0; i<MAX_PROJECTILES_ON_SCREEN; i++)//percorre o vetor de inimigos
    {
        if(object3[i].x == x_position && object3[i].y == y_position && object3[i].health_points>=1)//se a posição onde o projétil queria se mover é a mesma do inimigo
        {
            object3[i].health_points = object3[i].health_points - damage;//decrementa da vida do inimigo
        }
    }
}
...
int main(void)
{
    ...
    while (!WindowShouldClose()) //Roda enquanto não for fechada a tela
    {
        ...
        for(i=0; i<MAX_PROJECTILES_ON_SCREEN; i++)//percorre o vetor de projéteis
        {
            ...
            {
                for(j=0; j<MAX_ENEMIES; j++)//percorre o vetor de inimigos
                {
                    if(enemy[j].x==(projectile[i].x+projectile[i].dx)&&enemy[j].y==(projectile[i].y+projectile[i].dy)&&enemy[j].health_points>=1)//se a posição onde o projétil queria se mover é a mesma do inimigo
                    {
                        enemy[j].health_points--;//decrementa da vida do inimigo
                    }
                }
                damage_position(projectile[i].x+projectile[i].dx,projectile[i].y+projectile[i].dy ,1 ,&player, enemy, projectile);
