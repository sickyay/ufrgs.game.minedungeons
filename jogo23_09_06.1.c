...
bool draw_map (struct STRUCT_DATA *data)//funcao que le o que acontece na matriz do mapa
{
    ...
    {
        DrawRectangle(data->trap[i].x*SIDE, data->trap[i].y*SIDE, SIDE, SIDE, ORANGE);
    }
        ...
        if (!menu_open) //Roda enquanto o menu não for aberto
        {
            ...
            if(data.player.head_direction_x!=0||data.player.head_direction_y!=0)//se alguma das direções não for nula
            {
                if(data.player.intends_to_put_bomb==true&&data.player.bomb_inventory>0){
                    ...
                    shoot(&data,&data.player, &data.projectile[data.shot_counter], data.player.head_direction_x, data.player.head_direction_y);//atira
