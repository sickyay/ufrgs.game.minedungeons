    ...
    while (!WindowShouldClose()){//Roda enquanto não for fechada a tela
        ...
        if(menu_open){
            ...
            if(data.player.health_points==0){
                ...
                DrawText("You Died", (MAP_PIXEL_WIDTH/2)-200, (MAP_PIXEL_HEIGHT/2)-50, 100, WHITE);
                DrawText("Press M to open Menu", (MAP_PIXEL_WIDTH/2)-270, (MAP_PIXEL_HEIGHT/2)+80, 50, WHITE);
            }
        }
        EndDrawing();
    }
