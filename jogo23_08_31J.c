...
#define ENEMY_STEP_COOLDOWN 0.2
...
bool draw_map (char map_matrix[MAP_LINES][MAP_COLUMNS+1],struct STRUCT_STATS object, struct STRUCT_STATS object1, struct STRUCT_STATS trap_vector[])//funcao que le o que acontece na matriz do mapa
{
    ...
    for (int y = 0; y < MAP_LINES; y++)//colunas
    {
        for (int x = 0; x < MAP_COLUMNS; x++)//linhas
        {
            ...
            case '#'://parede
                DrawRectangle(x*SIDE, y*SIDE, SIDE, SIDE, (Color)
                {
                    0, 0, 0, 255
                });
                break;
    ...
    for(i=0; i<MAX_SMOKES; i++)
    {
        ...
        if(smoke_vector[i].x>=0&&smoke_vector[i].x<MAP_COLUMNS&&smoke_vector[i].y>=0&&smoke_vector[i].y<MAP_LINES)
        {
            if(smoke_vector[i].lifetime.timer_current - smoke_vector[i].lifetime.timer_start <= smoke_vector[i].lifetime.cooldown)
            {
                for(j=0; j<2; j++)
                {
                    DrawRectangle((smoke_vector[i].x*SIDE)+((rand()%(SIDE*2)-(SIDE/2))), (smoke_vector[i].y*SIDE)+((rand()%(SIDE*2)-(SIDE/2))), SIDE/2, SIDE/2, (Color)
                    {
                        255-(((smoke_vector[i].lifetime.timer_current-smoke_vector[i].lifetime.timer_start)*255)/smoke_vector[i].lifetime.cooldown), 0, 0, 255-(((smoke_vector[i].lifetime.timer_current-smoke_vector[i].lifetime.timer_start)*255)/smoke_vector[i].lifetime.cooldown)
                    });
                }
            }
        }
...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if(menu_open)
        {
            DrawText(TextFormat("GAME PAUSED"), MAP_PIXEL_WIDTH/2, MAP_PIXEL_HEIGHT/2, 40, RED);
        }
        else
        {
