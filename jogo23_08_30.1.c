...
#define MENU_PIXEL_WIDTH 300 //em pixels
#define MENU_PIXEL_HEIGHT 450 //em pixels
...
#define PLAYER_SHOOT_COOLDOWN 0.3 //cooldown do tiro do player
#define PLAYER_DAMAGE_IMUNITY_COOLDOWN 0.5

#define ENEMY_STEP_COOLDOWN 0.15
...
bool draw_menu (int selected_option){
    ...
    int i;
    int button_height = ((MENU_PIXEL_HEIGHT - (MENU_PIXEL_BORDER*7))/6);
    int menu_pos_x = (MAP_PIXEL_WIDTH/2)-(MENU_PIXEL_WIDTH/2);
    int menu_pos_y = (MAP_PIXEL_HEIGHT/2)-(MENU_PIXEL_HEIGHT/2);
    char options_text[5][125] = {{"N - New run"},{"L - Load save"},{"S - Save game"},{"Q - Quit game"},{"M - Close Menu"}};


    DrawRectangle(menu_pos_x, menu_pos_y, MENU_PIXEL_WIDTH, MENU_PIXEL_HEIGHT, (Color){125,125,125,255});
    //DrawText(TextFormat("GAME PAUSED"), MAP_PIXEL_WIDTH/2-150, MAP_PIXEL_HEIGHT/6, 40, RED);

    for(i = 1;i<6;i++){
        if(selected_option==i){
            DrawRectangle(menu_pos_x+MENU_PIXEL_BORDER, menu_pos_y +((button_height+MENU_PIXEL_BORDER)*i) + MENU_PIXEL_BORDER, MENU_PIXEL_WIDTH-(MENU_PIXEL_BORDER*2), button_height, (Color){0,0,0,255});
        }else{
            DrawRectangle(menu_pos_x+MENU_PIXEL_BORDER, menu_pos_y +((button_height+MENU_PIXEL_BORDER)*i) + MENU_PIXEL_BORDER, MENU_PIXEL_WIDTH-(MENU_PIXEL_BORDER*2), button_height, (Color){255,255,255,255});
        }
        DrawText(options_text[i-1], menu_pos_x+(MENU_PIXEL_BORDER)+button_height/5,  button_height/4+ menu_pos_y +((button_height+MENU_PIXEL_BORDER)*i) + MENU_PIXEL_BORDER, button_height/2, RED);
    }
    return true;
}
        ...
        if(stream_vector[i].x>=0&&stream_vector[i].x<MAP_COLUMNS&&stream_vector[i].y>=0&&stream_vector[i].y<MAP_LINES){
            if(stream_vector[i].lifetime.timer_current - stream_vector[i].lifetime.timer_start <= stream_vector[i].lifetime.cooldown){
                ...
                switch(stream_vector[i].type){
                    case 0:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y+(SIDE/2),effect_pos_x+(SIDE/2) ,effect_pos_y+SIDE , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        stream_vector[i].type = stream_vector[i].type + 1;
                        break;
                    case 1:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+SIDE , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        break;
                    case 2:
                        DrawLine(effect_pos_x, effect_pos_y+SIDE,effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        stream_vector[i].type = stream_vector[i].type + 1;
                        break;
                    case 3:
                        DrawLine(effect_pos_x+SIDE, effect_pos_y,effect_pos_x ,effect_pos_y+SIDE , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        break;
                    case 4:
                        DrawLine(effect_pos_x, effect_pos_y+(SIDE/2),effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        stream_vector[i].type = stream_vector[i].type + 1;
                        break;
                    case 5:
                        DrawLine(effect_pos_x, effect_pos_y+(SIDE/2),effect_pos_x+SIDE ,effect_pos_y+(SIDE/2) , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        break;
                    case 6:
                        DrawLine(effect_pos_x, effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        stream_vector[i].type = stream_vector[i].type + 1;
                        break;
                    case 7:
                        DrawLine(effect_pos_x, effect_pos_y,effect_pos_x+SIDE ,effect_pos_y +SIDE, (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        break;
                    case 8:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        stream_vector[i].type = stream_vector[i].type + 1;
                        break;
                    case 9:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+SIDE , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        break;
                    case 10:
                        DrawLine(effect_pos_x+SIDE, effect_pos_y,effect_pos_x+(SIDE/2) ,effect_pos_y+(SIDE/2) , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        stream_vector[i].type = stream_vector[i].type + 1;
                        break;
                    case 11:
                        DrawLine(effect_pos_x+SIDE, effect_pos_y,effect_pos_x ,effect_pos_y+SIDE , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        break;
                    case 12:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y+(SIDE/2),effect_pos_x +SIDE,effect_pos_y+(SIDE/2) , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        stream_vector[i].type = stream_vector[i].type + 1;
                        break;
                    case 13:
                        DrawLine(effect_pos_x, effect_pos_y+(SIDE/2),effect_pos_x+SIDE ,effect_pos_y+(SIDE/2) , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        break;
                    case 14:
                        DrawLine(effect_pos_x+(SIDE/2), effect_pos_y+(SIDE/2),effect_pos_x+SIDE ,effect_pos_y+SIDE , (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
                        stream_vector[i].type = stream_vector[i].type + 1;
                        break;
                    case 15:
                        DrawLine(effect_pos_x, effect_pos_y,effect_pos_x+SIDE ,effect_pos_y +SIDE, (Color){125, 125, 125, 255-(((stream_vector[i].lifetime.timer_current-stream_vector[i].lifetime.timer_start)*255)/stream_vector[i].lifetime.cooldown)});
...
int main(void)
{
    ...
    while (!WindowShouldClose()){
        ...
        if (!menu_open) //Roda enquanto não for fechada a tela
        {
            //===================================================================================================================
            //PLAYER
            ...
            for(i=0; i<MAX_ENEMIES; i++)//percorre o vetor de inimigos
            {
                ...
                if (enemy[i].step.timer_current - enemy[i].step.timer_start >= enemy[i].step.cooldown)//se o cooldown for zero, pega a intenção de movimento
                {
                    ...
                    {
                        if(!test_if_object_is_there (&enemy[i],map,enemy[i].dx,enemy[i].dy,'I'))
                        {
